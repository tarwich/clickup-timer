...
- **Stuck or just need a stopwatch?** Right-click the timer or its notification-area icon → **Use timer without ClickUp (discard recovery)**. This resets local recovery without needing OAuth or a ClickUp response. Start, Stop, and switching cached tasks then work locally; Start also works without a selected task. Local mode survives restart and never uploads time or reimports the stuck server timer. Record time manually in ClickUp; any timer still running there must be stopped there. Choose **Enable ClickUp logging** to return to connected mode explicitly.
...
- MCP cannot edit an existing time entry's stop time. A confirmed Stop accepts ClickUp's recorded duration and clears recovery, even after a delay. Differences from the requested cutoff do not block starting again or switching tasks; adjust recorded time manually if needed.
