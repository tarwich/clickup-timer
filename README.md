...
- Expired sign-in does not block **Reconnect to ClickUp** in Settings. Reauthorize the same account and include the timer's workspace. Saved stop requests retain their original cutoff and retry immediately after reconnect. A search-only authorization still cannot stop the timer; Settings reports that timer OAuth is required. You can stop the timer directly in ClickUp while that authorization is unavailable.
