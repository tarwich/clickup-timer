...
{
    public static Box Floating(Box work, double scale, double horizontal, double vertical)
    {
        var width = Math.Min(work.Width, (int)Math.Ceiling(336 * scale));
        var height = Math.Min(work.Height, (int)Math.Ceiling(40 * scale));
    ...
    public static Box? Find(Box bar, IReadOnlyList<Box> occupied, double scale, int? preferredX = null)
    {
        if (scale <= 0 || bar.Width <= bar.Height || bar.Height < 24 * scale) return null;
        var gap = (int)Math.Ceiling(6 * scale);
        var width = (int)Math.Ceiling(336 * scale);
        var height = Math.Min((int)Math.Ceiling(40 * scale), bar.Height - 4);
